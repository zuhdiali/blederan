...
<div class="container">
  <div class="page-inner">
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          <form action="{{route('admin-simpan-akomodasi')}}" method="POST" enctype="multipart/form-data">
          @csrf <!-- {{ csrf_field() }} -->
            <div class="card-header">
              <div class="card-title">Tambah Akomodasi</div>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group {{$errors->has('nama_akomodasi') ? 'has-error has-feedback' : ''}}">
                    ...
                    <input
                      ...
                      name="nama_akomodasi"
                      value="{{ old('nama_akomodasi') }}"
                    />
                    @if ($errors->has('nama_akomodasi'))
                    <small  class="form-text text-muted">{{ $errors->first('nama_akomodasi') }}</small>
                    @endif
                  </div>
                  ...
                  <div class="form-group {{$errors->has('gambar_akomodasi') ? 'has-error has-feedback' : ''}}">
                    ...
                    <small  class="form-text text-muted">{{ $errors->first('gambar_akomodasi') }}</small>
                    @endif
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group {{$errors->has('nama_pemilik') ? 'has-error has-feedback' : ''}}">
                    <label for="nama_pemilik">Nama Penjual</label>
                    <input
                      type="text"
                      class="form-control form-control"
                      id="nama_pemilik"
                    ...
                    <small  class="form-text text-muted">{{ $errors->first('nama_pemilik') }}</small>
                    @endif
                  </div>
                  <div class="form-group">
                    <label for="kontak_pemilik">Kontak Penjual</label>
                    <input
                      type="text"
                      class="form-control form-control"
                      id="kontak_pemilik"
                      ...
                      name="kontak_pemilik"
                      value="{{ old('kontak_pemilik') }}"
                    />
                    @if ($errors->has('kontak_pemilik'))
                    <small  class="form-text text-muted">{{ $errors->first('kontak_pemilik') }}</small>
                    @endif
                  </div>
                  <div class="form-group">
                    <label for="deskripsi">Deskripsi Akomodasi</label>
                    <textarea class="form-control" id="comment" rows="5" name="deskripsi" placeholder="Masukkan deskripsi akomodasi (opsional)">{{ old('deskripsi') }}</textarea>
            ...
            <div class="card-action">
              <button type="submit" class="btn btn-success">Tambah Akomodasi</button>
            </div>
          </form>
        </div>
      </div>
